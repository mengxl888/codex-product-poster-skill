...
4. Run the bundled helper. Prefer a prompt file when the prompt contains many quotation marks or newlines:
   ...
   The helper discovers environment variables at invocation time and also reads dimensions/quality written in the prompt; set environment variables in the current shell only when they are not already configured. Resolve `scripts/` relative to this skill directory when the current project is elsewhere. For example, `尺寸 1536x1024，质量 high` is converted into request fields; when neither is stated or configured, the request uses `auto`.
